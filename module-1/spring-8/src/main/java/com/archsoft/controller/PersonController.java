...
public class PersonController {
    ...
    public ResponseEntity<PersonTO> create(@RequestBody PersonTO personTO) throws RecordNotFoundException {
        ...
        return ResponseEntity.ok(personConverter.toTransferObject(person).applyHateoas());
    }

    @GetMapping("/{id}")
    public ResponseEntity<PersonTO> read(@PathVariable("id") Long id) throws RecordNotFoundException {
        PersonTO personTO = personConverter.toTransferObject(personService.read(id));

        return ResponseEntity.ok(personTO.applyHateoas());
    }
