...
* [`Message`](src/app/model/message.model.ts) - holds individual chat messages
* [`Thread`](src/app/model/thread.model.ts) - holds metadata for a group of `Message`s
* [`User`](src/app/model/user.model.ts) - holds data about an individual user
...
* [`MessagesService`](src/app/service/messages/messages.service.ts) - manages streams of `Message`s
* [`ThreadsService`](src/app/service/threads/threads.service.ts) - manages streams of `Thread`s
* [`UserService`](src/app/service/users/users.service.ts) - manages a stream of the current `User`

There are also three top-level components:

* [`ChatNavBar`](src/app/components/chat-nav-bar/chat-nav-bar.component.ts) - for the top navigation bar and unread messages count
* [`ChatThreads`](src/app/components/chat-threads/chat-threads.component.ts) - for our clickable list of threads
* [`ChatWindow`](src/app/components/chat-window/chat-window.component.ts) - where we hold our current conversation
